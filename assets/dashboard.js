let currentTab = 'home';
        ...
        function sortTable(type, colIdx) {
            ...
            if (type === 'rendafixa') tbodyId = 'rendafixa-tbody';
            ...
            else if (type === 'rendafixa' && (colIdx === 1 || colIdx === 6)) isNumeric = false;

            const key = type + ':' + colIdx;
            ...
            rows.sort((a, b) => {
                ...
                if (isNumeric) {
                    valA = Number(valA) || parseFloat(valA.replace(/[^0-9,.-]/g, '').replace(',', '.'));
                    valB = Number(valB) || parseFloat(valB.replace(/[^0-9,.-]/g, '').replace(',', '.'));
                    if (!Number.isFinite(valA)) valA = 0;
                    if (!Number.isFinite(valB)) valB = 0;
        ...
        */
        ...
        function renderRendaFixaPanel(data) {
            ...
            if (tbody) {
                if (tdData.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-secondary);padding:2rem;">Nenhum título disponível</td></tr>';
                    ...
                    tbody.innerHTML = tdData.map(td => {
                        ...
                        return '<tr onclick="openTdDetailModal(\'' + tdJson + '\')" style="cursor:pointer;" title="' + breakdownTip.replace(/"/g, '&quot;') + '">' +
                            '<td class="name-cell" data-sort="' + (td.general_rank || 9999) + '" style="font-weight:600;">' + generalRank + name + '</td>' +
                            '<td class="td-tipo">' + group + groupRank + '</td>' +
                            '<td class="font-mono tabular" data-sort="' + (yieldVal ?? -1) + '" style="font-weight:600;color:var(--positive);">' + yieldStr + '</td>' +
                            '<td class="font-mono tabular" data-sort="' + (td.historical_yield_percentile ?? -1) + '" title="Percentil da taxa atual no histórico do título">' + yieldPercentile + '</td>' +
                            '<td class="font-mono tabular" data-sort="' + (td.days_to_maturity ?? -1) + '" style="font-size:0.8rem;">' + maturity + '</td>' +
                            '<td data-sort="' + score + '"><span class="score-pill ' + getScoreRangeClass(score) + '" style="font-size:0.75rem;height:1.5rem;min-width:1.5rem;">' + formatScore(score) + '</span></td>' +
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    label: function(context) {
                                        return 'Atratividade em ' + context.label + ': ' + context.parsed.y.toFixed(1);
                                    }
                                }
                            },
                            valueLabels: {
