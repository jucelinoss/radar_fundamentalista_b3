...
def setup_windows_schedule():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.info(f"   Will run daily at 08:00")
        else:
...
def setup_linux_schedule():
    """Register a daily cron job (Linux/macOS)."""
    python_exe = sys.executable
    pipeline_script = os.path.join(SRC_DIR, "pipeline.py")
    cron_line = f"0 8 * * 1-5 {python_exe} {pipeline_script} >> {LOG_DIR}/cron.log 2>&1"
...
def show_report():
    ...
    import database
    try:
        logs = database.get_pipeline_history(limit=15)
        if not logs:
        ...
        for i, log in enumerate(logs, 1):
            stocks = f"{log['stocks_ok']}✅/{log['stocks_fail']}❌"
            fiis = f"{log['fiis_ok']}✅/{log['fiis_fail']}❌"
            fiagros = f"{log['fiagros_ok']}✅/{log['fiagros_fail']}❌"
            dur = f"{log['duration_seconds']:.1f}s" if log['duration_seconds'] else "N/A"
            started = log.get('started_at', '')[:19] if log.get('started_at') else 'N/A'
        ...
        # Also show last update timestamp
        last_ts = database.get_last_update_timestamp()
        if last_ts:
            ...
            if hours_ago < 24:
                ...
                print(f"  ⚠️  Data is stale ({hours_ago:.1f}h old) — consider running the pipeline")
        print()
...
def main():
    ...
    parser.add_argument(
        ...
        help="Only fetch/update data, skip dashboard generation"
    )
    parser.add_argument(
        "--generate-only", action="store_true",
        help="Only regenerate dashboard from existing database"
    )
    parser.add_argument(
        "--schedule", action="store_true",
        help="Register a daily scheduled task (Windows: schtasks, Linux: cron)"
    )
    parser.add_argument(
        "--daemon", action="store_true",
        help="Run continuously with 24h interval (for Docker/always-on)"
    )
    parser.add_argument(
        "--report", action="store_true",
        help="Show pipeline execution history and exit"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Export all assets to CSV files in data/"
    )
    parser.add_argument(
        "--export-json", action="store_true",
        help="Export all assets to a single JSON file"
    )
    parser.add_argument(
        "--export-top-picks", action="store_true",
        help="Export top picks for AI/portfolio analysis"
    )
    parser.add_argument(
        "--export-all", action="store_true",
        help="Export all formats (CSV + JSON + top picks)"
    )
    parser.add_argument(
